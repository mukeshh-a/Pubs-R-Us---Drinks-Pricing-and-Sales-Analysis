...
SELECT p.pub_name, b.beverage_name, b.price_per_unit, s.quantity, (b.price_per_unit * s.quantity) AS total_sales_amount
FROM pubs p
JOIN sales s ON p.pub_id = s.pub_id
JOIN beverages b ON s.beverage_id = b.beverage_id
...
SELECT p.pub_name, s.transaction_date,
    ...
    WHERE s2.pub_id = p.pub_id AND s2.transaction_date <= s.transaction_date) AS running_total_sales_amount
FROM pubs p
JOIN sales s ON p.pub_id = s.pub_id
...
SELECT p.pub_id, p.pub_name, b.category,
       (SUM(s.quantity * b.price_per_unit) / pt.pub_total_sales_amount) * 100 AS percentage_contribution,
       pt.pub_total_sales_amount AS pub_total_sales_amount
FROM pubs p
JOIN sales s ON p.pub_id = s.pub_id
JOIN beverages b ON s.beverage_id = b.beverage_id
JOIN (
